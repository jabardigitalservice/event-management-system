<template>
    ...
    <div class="font-lato text-gray-800">
      <slot name="header" />
      <label v-show="label" class="message-notif-form__label-required text-gray-800">{{label}}</label>
      ...
      <div class="mt-2 flex w-full items-center justify-center">
        <div
          ...
          :class="[
            fileIsCorrect ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50',
            props.heightDragAndDrop
          ]"
          class="flex w-full flex-col justify-center rounded-lg border-2 border-dashed px-4"
        >
          <div class="mb-3 flex items-center justify-center">
            <NuxtIcon name="common/document" class="h-9 w-9 text-4xl text-gray-600"/>
          </div>
  
          <div class="flex w-full flex-row items-center justify-between">
            <div class="w-[85%]">
              <p class="mb-2 truncate font-lato text-[14px] font-normal text-black">
                {{ dataFiles.name }}
              </p>
              ...
              <p v-if="!FileSizeIsCompatible()" class="font-lato text-[11px] font-bold text-red-600">
                {{ props.detailDragAndDrop.informationSizeCompatible }}
              </p>
  
              <p v-if="!FormatFileIsCompatible()" class="font-lato text-[11px] font-bold text-red-600">
            ...
            <div class="flex flex-row">
              ...
              <button class="w-4 ml-5" @click.prevent="previewFile">
                <NuxtIcon v-if="!proggresBarIsSuccess" name="common/eyes" class="h-4 w-4 text-green-600"/>
              </button>
            </div>
          </div>
        </div>
        ...
        >
          <div class="flex flex-col items-center justify-center">
            <NuxtIcon name="common/upload-file" class="mb-3 text-4xl text-gray-300" />
            <p class="mb-2 font-lato text-[14px] text-gray-700">
              <span>Drag File kesini</span>
              <span class="text-gray-500 ml-1">atau</span>
              <span class="text-green-600 underline ml-1">Pilih File</span>
            </p>
          </div>
          <input
