<template>
  <div class="flex justify-between gap-6">
    <div class="flex-1 font-lato text-gray-800 w-50">
      <slot name="header" />
    ...
    <div class="flex-1 w-50 mt-5 max-h-[300px] overflow-auto pl-3 pt-5 pr-3">
      <RadioGroup v-show="dataFilesMultiple" v-model="selected">
        <RadioGroupLabel class="sr-only">Server size</RadioGroupLabel>
        <div class="space-y-2 grid lg:grid-cols-3 md:grid-cols-2 gap-4">
          <RadioGroupOption
...
const convertFileToBase64 = (FileObject: Blob, name: string) => {
  ...
  reader.onload = () => {
    dataFilesMultiple.value.filter(x => x.name == name)[0].data = reader.result.split(',')[1];
