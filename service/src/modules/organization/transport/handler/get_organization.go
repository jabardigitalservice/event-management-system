...
func (h *Handler) GetOrganizations(w http.ResponseWriter, r *http.Request) {
	...
	validSortOptions := []string{"name", "email", "created_at", "updated_at"}
	...
	responseOrganizations, count, err := h.endpoint.GetOrganizations(ctx, queryParam)
	if err != nil {
		RenderError(w, r, h, err, responseOrganizations)
		return
	}
